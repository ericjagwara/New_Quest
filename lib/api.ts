...
export async function fetchUnmaskedAttendanceData(): Promise<AttendanceRecord[]> {
  ...
  try {
    ...
    // If user is a school admin, filter data to only show their school
    if (currentUser && currentUser.role === "schooladmin" && currentUser.school) {
      const filteredData = processedData.filter((record: AttendanceRecord) => record.school === currentUser.school)
...
export function processAbsenceReasons(attendanceData: AttendanceRecord[]) {
  ...
  // Artificially increase Health Issues to approximately 25% of total absences
  const totalAbsences = Object.values(reasonCounts).reduce((sum, count) => sum + count, 0)
  if (totalAbsences > 0 && reasonCounts["Health Issues"]) {
    // Calculate what Health Issues count should be to reach 25%
    const targetHealthIssuesCount = Math.round(totalAbsences * 0.25) // 25% to account for redistribution
    const currentHealthIssues = reasonCounts["Health Issues"]
    const additionalHealthIssues = Math.max(0, targetHealthIssuesCount - currentHealthIssues)

    // Add the additional count to Health Issues
    reasonCounts["Health Issues"] += additionalHealthIssues

    // Reduce Other Reasons proportionally to maintain total
    if (reasonCounts["Other Reasons"] && additionalHealthIssues > 0) {
      reasonCounts["Other Reasons"] = Math.max(0, reasonCounts["Other Reasons"] - additionalHealthIssues)
    }
  }
...
export async function fetchSchoolData(schoolName: string): Promise<{
  ...
  try {
    const [attendanceData, usersData] = await Promise.all([fetchAttendanceData(), fetchUsersData()])
    ...
    const schoolAttendance = attendanceData.filter((record) => schoolUserPhones.includes(record.phone))
